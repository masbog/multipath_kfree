...
char* http_ls(char *cwd, uint32_t* sz)
{
    ...
    {
        printf("You're running on iPhone X (%s) iOS 11.3.1 build: %s\n", kvers.machine, build);
    }
    ...
    {
        sprintf(data,
                "<html><h1>\tListing [%s] on %s - %s</h1><br>\n"
                "<html><h1>\tKernel Version: %s</h1><br>\n"
                ...
                cwd, myhost, kvers.machine, kvers.version);
    } else {
        sprintf(data,
